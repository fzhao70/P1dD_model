...
def pde_solver(time_step, level, c, v, w, TTL = 25, \
                Dilution_type = 'None', H_Dissipate = True, V_Sedimentation = False, V_Transportation = False):
    """
    ...
    for modal in range(modal_number):
        ...
        for time in range(0, time_step - 1):
            ...
            if any(c[modal, time + 1, :] > level * 4 * modal):
                c[modal, time + 1, :] = 999 * c[modal, time + 1, :]
                exceed_flag = 1
                break
        # Init A ------>  time + 1
            A = np.diagflat([D_zz[modal] * delta_t for i in range(level - 1)], -1)
            A = A + np.diagflat([D_zz[modal] * delta_t  for i in range(level - 1)], 1)
            A = A + np.diagflat(\
        ...
        # Init B ------> time
            B = np.diagflat([-1 * delta_z * delta_t * w[time, i] - D_zz[modal] * delta_t  for i in range(level - 1)], -1)
            B = B + np.diagflat([delta_z * delta_t * w[time, i + 1] - D_zz[modal] * delta_t for i in range(level - 1)], 1)
            B = B + np.diagflat(\
